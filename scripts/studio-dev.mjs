...
const CONTRACT = path.join(PROJECT, "contracts", "concord_batch.py");
...
async function main() {
  ...
  const accountingRaw = await client.readContract({ address, functionName: "get_accounting" });
  const accounting = JSON.parse(String(accountingRaw));
  if (accounting.total_received !== "0" || accounting.total_locked !== "0" || accounting.total_credits !== "0" || accounting.total_withdrawn !== "0") {
    throw new Error("fresh deployment accounting smoke check failed");
  }
  const evidence = {
    network: "studio-dev",
    chainId: EXPECTED_CHAIN_ID,
    rpc: endpoint,
    contract: "ConcordBatch",
